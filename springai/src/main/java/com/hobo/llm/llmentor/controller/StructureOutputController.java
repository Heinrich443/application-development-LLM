...
public class StructureOutputController implements InitializingBean {
    ...
    @GetMapping("/list")
    public String convertList(String message) {
        List<Book> book = chatClient.prompt("请给我推荐几本python有关的书")
    ...
    @GetMapping("/map")
    public String convertMap(String message) {
        // 会抛异常
        Map<String, Book> book = chatClient.prompt("请给我推荐几本python有关的书")
                .call().entity(new ParameterizedTypeReference<Map<String, Book>>() {});
        System.out.println(book.toString());
        return book.toString();
    }

    @GetMapping("/map1")
    public String convertMap1(String message) {
        Map<String, Object> book = chatClient.prompt("请给我推荐几本python有关的书")
